...
function aStar(params) {
  ...
  while (openHeap.size()) {
    ...
    var neighbors = params.neighbor(node.data, reconstructPath(node));
    for (var i = 0; i < neighbors.length; i++) {
      ...
      var hFromThisNode = node.h + params.heuristic(node.data);
...
//      console.log('current: ' + neighborNode.f, 'best: ' + bestNode.f);
      if (neighborNode.f < bestNode.f) bestNode = neighborNode;
      if (update) {
        openHeap.heapify();
      } else {
